<#import "/spring.ftl" as spring />
<#import "../../admin-base.ftl" as layout>
<#import "../../admin-components.ftl" as admin>

<#macro headers>
    <link rel="stylesheet" href="${resourcePrefix}/front/admin/assets/vendor/daterangepicker/daterangepicker.css">
</#macro>

<#macro attendanceSearch id searchPrompt>
    <form id="filterForm" onsubmit="return false;">
        <div class="form-group row">
            <label class="col-sm-2 col-form-label">
                <@spring.message code="membership.date-range" />
            </label>
            <div class="col-sm-10">
                <button id="filterDateRange" class="btn btn-white">
                    <i class="tio-date-range mr-1"></i>
                    <span class="filterDateRange-preview"></span>
                </button>
            </div>
        </div>
        <div class="form-group row">
            <label class="col-sm-2 col-form-label toggle-switch mb-3" for="${id}IncludeUnregistered">
                <@spring.message code="membership.include-unregistered" />
            </label>
            <label class="toggle-switch mx-2" for="${id}IncludeUnregistered">
                <input type="checkbox" class="js-toggle-switch toggle-switch-input"
                       id="${id}IncludeUnregistered"
                       name="${id}IncludeUnregistered"
                       data-hs-toggle-switch-options='{
                        "isChecked": true
                       }'
                       checked="checked"
                />
                <span class="toggle-switch-label">
                    <span class="toggle-switch-indicator"></span>
                </span>
            </label>
        </div>
        <!-- Search -->
        <div class="form-group row">
            <label class="col-sm-2 col-form-label">
                <@spring.message code="membership.attendance-search-terms" />
            </label>
            <div class="col-sm-10 input-group input-group-merge input-group-flush">
                <div class="input-group-prepend">
                    <div class="input-group-text">
                        <i class="tio-search"></i>
                    </div>
                </div>
                <input
                        id="${id}Search"
                        type="search"
                        class="form-control"
                        placeholder="<@spring.messageText code=searchPrompt text=searchPrompt />..."
                        aria-label="<@spring.messageText code=searchPrompt text=searchPrompt />">
            </div>
        </div>
        <!-- End Search -->
    </form>
</#macro>

<#macro attendanceDataFunction id>
function(data) {
    data.filter = {
      from: start,
      to: end,
      includeUnregistered: $('#${id}IncludeUnregistered').is(":checked")
    };
    console.log(data);
    return JSON.stringify(data);
}
</#macro>

<#macro dataScript>
    <script src="${resourcePrefix}/front/admin/assets/vendor/daterangepicker/moment.min.js"></script>
    <script src="${resourcePrefix}/front/admin/assets/vendor/daterangepicker/daterangepicker.js"></script>

    <script>
        let report;
        let start = moment().subtract(29, 'days');
        let end = moment();

        function displayDate(row, type, set, meta) {
            if (row.data.time) {
                return new Date(row.data.time * 1000).toLocaleDateString();
            }
            return "";
        }

        function displayTime(row, type, set, meta) {
            if (row.data.time) {
                return new Date(row.data.time * 1000).toLocaleTimeString();
            }
            return "";
        }

        function refreshTable() {
            if (typeof(report) === 'object') {
                report.search($("#attendanceTableSearch").val()).draw();
            }
        }

        function dateRangeCallback(selectedStart, selectedEnd) {
            $('#filterDateRange .filterDateRange-preview')
                .html(selectedStart.format('MMMM D, YYYY') + ' - ' + selectedEnd.format('MMMM D, YYYY'));
            start = selectedStart
            end = selectedEnd
            refreshTable();
        }

        function onPageLoad() {
            $.HSCore.components.HSDaterangepicker.init($('#filterDateRange'), {
                startDate: start,
                endDate: end,
                ranges: {
                    'Today': [moment(), moment()],
                    'Yesterday': [moment().subtract(1, 'days'), moment().subtract(1, 'days')],
                    'Last 7 Days': [moment().subtract(6, 'days'), moment()],
                    'Last 30 Days': [moment().subtract(29, 'days'), moment()],
                    'This Month': [moment().startOf('month'), moment().endOf('month')],
                    'Last Month': [moment().subtract(1, 'month').startOf('month'), moment().subtract(1, 'month').endOf('month')]
                }
            }, dateRangeCallback);

            dateRangeCallback(start, end);

            <@admin.adminSspTableScript
                id="attendanceTable"
                selectable=false
                columns=columns
                ajaxSrc="attendance-report/get-data"
                dataFunction=attendanceDataFunction
            />
            report = attendanceTable;

            $("#attendanceTableIncludeUnregistered").change(function() {
                refreshTable();
            });

            $("#export-google").click(function() {
                $("#export-open").hide();
                $('#export-status').html('<p class="spinner-border"></p><p>Exporting to your Google Drive, please wait...</p>');
                $('#export-modal').modal();
                $.ajax({
                    url: "./export",
                    type: "GET",
                    "contentType": "application/json",
                    beforeSend: function (request) {
                        request.setRequestHeader(header, token);
                    }
                }).error(function(jqXHR, textStatus, errorMessage) {
                    $('#export-status')
                        .html("<p><b>Export error!</b></p><p>An error occured performing export.</p><p>Please contact administrator</p>");
                }).done(function (data) {
                    $('#export-status')
                        .html("<p><b>Export complete!</b></p><p>Saved to your private Google Drive folder, to share move the sheet to appropriate shared drive.</p><p>Click below to open.</p>");
                    $('#export-open').attr("href", data.location);
                    $("#export-open").show();
                });
            });
        }
    </script>
</#macro>
<#macro export>
    <div class="d-sm-flex justify-content-sm-end align-items-sm-start">
        <a id="export-google" class="btn btn-soft-success mb-2" href="javascript:">
            <img
                    class="avatar avatar-xss avatar-4by3 mr-2"
                    src="${resourcePrefix}/front/admin/assets/svg/brands/google-sheets.svg"
                    alt="<@spring.message code="membership.export" />">
            <@spring.message code="membership.export" />
        </a>
    </div>
</#macro>

<@layout.mainLayout script=dataScript headers=headers>
    <@admin.adminSspTableCard
        id="attendanceTable"
        selectable=false
        searchable=true
        defaultPageLength=50
        pageLengths=[10,50,100]
        title="membership.attendance"
        columns=columns
<#--        buttons=export-->
        searchForm=attendanceSearch
        searchPrompt="membership.attendance-search"
    />

    <div id="export-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="false">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <!-- Header -->
                <div class="modal-top-cover bg-primary text-center">
                    <figure class="position-absolute right-0 bottom-0 left-0" style="margin-bottom: -1px;">
                        <svg preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 1920 100">
                            <path fill="#fff" d="M0,0c0,0,934.4,93.4,1920,0v100.1H0L0,0z"/>
                        </svg>
                    </figure>

                    <div class="modal-close">
                        <button type="button" class="btn btn-icon btn-sm btn-ghost-light" data-dismiss="modal" aria-label="Close">
                            <svg width="16" height="16" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                                <path fill="currentColor" d="M11.5,9.5l5-5c0.2-0.2,0.2-0.6-0.1-0.9l-1-1c-0.3-0.3-0.7-0.3-0.9-0.1l-5,5l-5-5C4.3,2.3,3.9,2.4,3.6,2.6l-1,1 C2.4,3.9,2.3,4.3,2.5,4.5l5,5l-5,5c-0.2,0.2-0.2,0.6,0.1,0.9l1,1c0.3,0.3,0.7,0.3,0.9,0.1l5-5l5,5c0.2,0.2,0.6,0.2,0.9-0.1l1-1 c0.3-0.3,0.3-0.7,0.1-0.9L11.5,9.5z"/>
                            </svg>
                        </button>
                    </div>
                </div>
                <!-- End Header -->

                <div class="modal-top-cover-avatar border-0">
                    <img class="avatar avatar-lg avatar-centered" src="${resourcePrefix}/front/admin/assets/svg/brands/google-sheets.svg" alt="Sheets">
                </div>

                <div class="modal-body border-top-0">
                    <div id="export-status" class="text-center">
                        ...
                    </div>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-white" data-dismiss="modal">
                        <@spring.message code="membership.export-cancel" />
                    </button>
                    <a id="export-open" class="btn btn-primary" target="_blank">
                        <@spring.message code="membership.open-export" />
                    </a>
                </div>
            </div>
        </div>
    </div>
</@layout.mainLayout>